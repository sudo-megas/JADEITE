...
import { parseQuantity } from '@shared/section3/units'
import { parseAmount } from '@shared/money'
import type { AppLanguage } from '../../i18n/format.js'
import { formatDate } from '../../i18n/format.js'
import { MoneyCell, QuantityCell } from './Cells.js'
      ...
      <table className="s3-ledger" data-testid="s3-ledger">
        ...
        <tfoot>
          <tr className="s3-ledger-totals" data-testid="s3-ledger-totals">
            ...
            <td colSpan={3} className="s3-figure">
    ...
    >
      ...
      <td className="s3-figure">
        <QuantityCell
          ...
          unit={type.unit}
          language={language}
          label={t('section3.quantity')}
          testId={`s3-quantity-${id}`}
          onCommit={(quantity) => handlers.onPatch(id, { quantity })}
        />
      </td>
 ...
 *
 * Date, type, direction and person carry forward from the row just committed;
 * quantity, price, source and note clear. That is the split a run of purchases
 * actually wants: they are usually the same person buying the same thing, and
 * never the same amount at the same price.
 */
function carriedForward(previous: LedgerRow | null, types: readonly ValuableType[]): Composed {
...
function AppendRow({
  ...
  const { t } = useTranslation()
  const [composed, setComposed] = useState<Composed>(() => carriedForward(previous, types))
  ...
  async function submit(): Promise<void> {
    ...
    const quantity = parseQuantity(composed.quantity, type.unit, language)
    if (quantity.kind !== 'quantity') {
      setProblem(
        quantity.kind === 'empty'
          ? 'section3.parse.REQUIRED'
          : `section3.parse.${quantity.reason}`
      )
      return
    }
    ...
    <tr className="s3-row s3-append" data-testid="s3-append-row" onKeyDown={onKeyDown}>
      ...
      <td className="s3-figure">
        <input
          ...
          type="text"
          inputMode="decimal"
          aria-label={t('section3.quantity')}
          value={composed.quantity}
          data-testid="s3-new-quantity"
