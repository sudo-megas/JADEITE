...
function cleanQuantity(quantity: unknown): number {
  if (typeof quantity !== 'number' || !Number.isSafeInteger(quantity)) fail('INVALID_QUANTITY')
  // `quantity > 0` is also a CHECK constraint; refused here so the reason
  // survives as a code the interface can name.
  if (quantity <= 0 || quantity > MAX_QUANTITY) fail('INVALID_QUANTITY')
  return quantity
}
...
export function readLedger(db: DatabaseType): LedgerData {
  ...
  const transactions = (
    db
      .prepare(
        `SELECT seq, date, date_provisional, type_code, direction, quantity, unit_price,
...
export function addTransaction(db: DatabaseType, draft: TransactionDraft): number {
  const run = db.transaction(() => {
    ...
    const quantity = cleanQuantity(draft.quantity)
    ...
    const result = db
      .prepare(
        `INSERT INTO s3_transactions
              (date, date_provisional, type_code, direction, quantity, unit_price,
               source, person_id, note)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
      )
      .run(
...
export function updateTransaction(db: DatabaseType, patch: TransactionPatch): void {
  const run = db.transaction(() => {
    const existing = db
      .prepare('SELECT seq, type_code FROM s3_transactions WHERE seq = ?')
      .get(patch.seq) as { seq: number; type_code: string } | undefined
    ...
    if (patch.quantity !== undefined) set('quantity', cleanQuantity(patch.quantity))
