...
**Project:** JADEITE — *Ekonomi Defteri* / *Economy Journal*: one encrypted book of what is held, what is owed and what is earned
**Copyright:** sudo-megas · **Licence:** GPL-3.0 · Free and Open-Source Software
**Status:** Specification v3 · amended 1 August 2026 (supersedes v2) · Companion document: `REALISATION.md`
**Amendments:** 2026-07-29 — §4.1, §5.1, §7.3, §16.6, §17, §19 (configuration split into two files; the Section 2 freeze made explicit and reversible; point revisions), settled during Realisations II and IV. · 2026-07-30 (Realisation VII) — §3.3, §5.3, §8.2, §8.3, §14.1, §14.3, §16.2, §19 (two allowlisted hosts and where the session governs them; ziynet struck from the closed list; the socket's decimal format; identification is impossible; the price ceiling). · 2026-07-30 — §1, §15, §16.2, §18, §19, §20 (the migration importer is retired before construction; the ladder ends at Realisation XI; nothing ships that only this owner could use; the `.jbk` importer's remit is backup, restore, copy and merge). · 2026-07-31 (point revision v0.8b, after the owner's first use of the built application) — §5.3, §7.1, §7.3, §9, §12.3, §13, §19 (Section 2 loses its year, its rollover and its archive; Section 4 loses the label and becomes a grid of value boxes; dates read `GG/AA/YYYY`). · 2026-07-31 (pre-Realisation IX survey) — §20 (what open item Q2 actually gates, measured against schema v3; **no ruling made** — the question is narrowed, not answered). · 2026-07-31 (Realisation IX) — §5.3, §15, §19, §20 (Q2 closed: merge is per-section choice, and the container carries what it will need; the `.jbk` layout; schema v5 adds a vault identity and per-section edit stamps; the restore door sits outside the lock; §3.3 gains the settings allow-list, the no-exempt-handler rule and the serialised ceremonies, all three from the hardening pass rather than from the design). · 2026-07-31 (point revision v0.9b) — §12.2, §17.1 (the application mark is artwork rather than a palette token, and is exempt by name; the Hakkında page, its compiled-in facts, and why its addresses are text). · 2026-07-31 (point revision v0.9c) — §12.2, §17.1 (the mark at 66px and the rail head that had to stack to hold it; the application names itself *Ekonomi Defteri* on screen, and the outward strings follow at Realisation X). · 2026-07-31 (point revision v0.9d) — front matter, §1, §17.1 (the outward strings follow the on-screen name sooner than X: the `Project:` line and the mission sentence stop saying *tracker*, and the launcher, the deb and the pacman package all carry *Ekonomi Defteri* / *Economy Journal*). · 2026-07-31 (Realisation X) — §4.1, §13 (two sentences that had quietly stopped being true: the config directory holds one *JADEITE*-managed file and Electron's own profile beside it, and the app language lives in `config.json` rather than in the vault, as §4.1 and §16.6 have said since the split. Both are corrections of the specification to the shipped design, not changes to it. §3.3 is **not** amended: `will-navigate` was widened past what §3.3 states and the code was narrowed back). · 2026-08-01 (Realisation XI) — §5.1 (the Windows data directory moves to `%LOCALAPPDATA%\jadeite\`, and configuration stays at `%APPDATA%\jadeite\`. One directory had been named twice, which made §4.1's two-file rule unsatisfiable on the port and put the vault among the entries Electron's own profile keeps there. A correction of the specification to the shipped design, and settled before the first Windows installer so that nothing has to be migrated. The same entry records that `0600` on `config.json` is inherited from the profile ACL on Windows rather than asserted by a `chmod`, which is a no-op there). · 2026-08-01 (Realisation XI, Linux wrap-up) — §4.1 (the config directory holds *at most* one JADEITE-managed file, not exactly one: `config.json` is written lazily and a defaults-only machine has none. Measured on a live session and filed by `docs/livecheck-results.md`; a correction of the specification to the shipped design, and the invariant that matters — JADEITE writes one configuration file and no other — is unchanged).
...
`contextIsolation: true`, `nodeIntegration: false`, sandboxed renderers, strict CSP, no remote content of any kind, preload exposes a minimal typed IPC surface only.

**Amendment of 31 July 2026 (Realisation IX's hardening pass) — three rules the surface review added.** *(1)* **The settings channel is allow-listed in both directions.** It was generic over any key, which was true of the table and wrong as a contract: from schema v5 the vault holds a lineage id and four trigger-kept edit stamps (§5.3), and a renderer able to write them could make this vault's own backups demand a credential, contradicting §4.4's first row. Readable and writable are two lists, and writable is the wider one. *(2)* **No handler is exempt from the guard**, including the two that answer while locked. `config:set` was the exception and it leaked an absolute path out of a failed write, because an Electron handler's exception is serialised into the renderer's rejected `invoke()`. *(3)* **Credential ceremonies are serialised.** Four channels run a 256 MiB Argon2id derivation and all four are callable as fast as a renderer likes; eight at once were measured at 1268 MiB. They queue rather than refuse — the cost is a wait, and §3.4 excludes the Argon2id time from every budget it sets. Network egress is limited to the **two** allowlisted price-provider hosts of §14.1 — the socket lives on a different machine from the history endpoint — and nothing else. No telemetry, no crash reporting, no update checks.
...
The **config directory** (per-OS, §5.1) contains **at most one** **JADEITE-managed** file — `config.json` is written lazily, on the first setting that departs from a default, so a machine whose owner has never changed palette or language has none:

- `config.json` — general application configuration, unencrypted: **the active palette and the app language, and nothing else.**

*(Amended at Realisation X. This read "exactly one app-managed file", and that was false on disk from Realisation I onward: `app.setName('jadeite')` makes Electron resolve `appData` → `userData` → `sessionData` to this same directory, so Chromium's profile is written beside `config.json` — `Cookies`, `Local State`, `Preferences`, `Network Persistent State`, `Cache/`, `GPUCache/`, `Crashpad/` and the rest. Measured on a machine that has run the application: **22 entries, of which `config.json` is one and Chromium wrote the other 21**, plus the singleton lock files `app.requestSingleInstanceLock()` adds while it is running. The sentence is corrected rather than the layout: relocating `userData` at v1.0 would move the provider session cookie of §14.2, and every one of those files belongs to the runtime rather than to this application. The distinction that matters is unchanged and is what the rules below are about — **JADEITE writes one configuration file, and no other.** Rule 2 is what makes the runtime's directory harmless: nothing about money is ever written here by anyone.)*
...
**Amendment of 1 August 2026 (Realisation XI) — the Windows vault leaves Roaming.** Both Windows lines above read `%APPDATA%\jadeite\`, which named one directory twice. The line was written before the configuration split of 2026-07-29 had a second directory to place, and it survived that split by oversight: it made Windows the only platform on which the data directory and the configuration directory are the same place, which §4.1 forbids in the sentence ending "so the data directory still holds exactly the two files named above". The collision was not merely nominal — `app.setName('jadeite')` resolves Electron's own profile to `%APPDATA%\jadeite` on Windows exactly as it resolves it to `~/.config/jadeite` on Linux, so the two vault files would have been written among `config.json` and every runtime entry Chromium keeps there. **The data directory on Windows is therefore `%LOCALAPPDATA%\jadeite\`, and the configuration directory stays at `%APPDATA%\jadeite\`**, which is where the runtime writes its profile in any case. Linux does not move: there the two have been distinct since Realisation I. This is a correction of the specification to the shipped design, not a change to it.

Two further reasons are recorded so the choice is not re-argued later as taste. **`%APPDATA%` is the roaming half of the profile**, copied to a domain share by machinery this application neither controls nor can audit; an encrypted vault leaving the machine unasked is still a copy leaving the machine, and §15 rules that backup destinations are local paths the owner chooses. **And a vault is per-machine by design** — the rig-to-laptop workflow is a `.jbk` carried by hand, so the one file that must never follow a user between machines by itself is precisely this one.

Nothing has to be migrated, which is why this was settled at XI rather than after it shipped. Realisation XI is the first Windows build, so no vault had ever been written to `%APPDATA%\jadeite`; the same correction after an installer had shipped would have cost a migration running before the lock screen, in the one place where failure locks the owner out of their own record. A container carries no path, so a `.jbk` written on Linux restores into the Windows location without knowing it changed.

**One thing this amendment exposes and does not close.** §4.1 requires `config.json` be written atomically and owner-only (`0600`). On Windows the `fs` mode argument only toggles the read-only attribute, so a `chmod` to `0600` is a silent no-op there and the file's protection comes entirely from the per-user ACL inherited from the profile directory. That is adequate — `%LOCALAPPDATA%` and `%APPDATA%` are inside the user profile, which is not readable by other unprivileged users — but it is inherited rather than asserted, and the port should not claim a mode it did not set. The suites that assert `0600` skip on Windows for this reason rather than pretending to pass.
...
*(Amended 31 July 2026, Realisation IX.)* **Schema v5** adds no table. `backup_log`, reserved since v1, is finally written to. Two facts a vault never needed until it could be copied arrive as settings rows: `vault_id`, sixteen random bytes minted once and never rewritten, and `s1_touched_at` … `s4_touched_at`, kept by triggers on the nine tables that carry owner edits. The three tables deliberately excluded are `s3_prices_live`, `s3_price_fetch` and `valuable_types` — the first two are written by the provider on a timer (§14), so counting them would mark Section 3 as edited every fifteen minutes whether or not the owner had touched it, and the stamp exists to answer *which machine holds the newer work*, a question a background fetch has no opinion about.
...
*(Amended 31 July 2026, point revisions v0.9b and v0.9c.)* **The application mark is the one thing on screen that is not a token, and it is exempt by name rather than by oversight.** Point revision v0.8b drew the mark as a vector that took its colour from the palette, so that it would be native in all ten. The owner's ruling is that a token-tinted diagram reads as a placeholder beside the rest of the application, and that the app should wear the artwork it was drawn for: `build/innerAPP.png` in the rail and on the ceremony panels, `build/outerAPP.png` in launchers and on the Hakkında page. Both are fixed jade and gold in every palette. The rule above is unchanged for everything it was written about — colour *written in code* — and `scripts/audit-colours.mjs` still fails the build on a literal in any `.ts`, `.tsx`, `.css` or `.html`. What is retired is the argument that the mark itself had to follow the palette.
...
- **Manual switching only.** The app must never read the OS locale, and must never change language on its own — the owner's explicit prohibition. Language is a setting in **`config.json`** (§4.1), defaulting to Turkish on first run. *(Amended at Realisation X. This said "inside the vault", which the configuration split of 2026-07-29 had already made false: §4.1 puts the palette and the language in `config.json` and §16.6 repeats it, for the reason the split exists — a setting the lock screen needs cannot live behind the lock. §4.1's rule 1 forbids mirroring it into the vault, so the two sentences could not both be obeyed, and the code has followed §4.1 since Realisation II. This clause was the straggler; the default half of it was always honoured.)*
...
- **The importer's remit is backup, restore, copy and merge** (owner's ruling, 30 July 2026) — moving one JADEITE vault's data to another JADEITE, and nothing else. It carries no feature that reads or explains a foreign document. Full replacement is specified above and lands in Realisation IX; **merge** is settled as *per-section choice* (Q2, closed 31 July 2026) and is **not built** in Realisation IX. The container carries what merge will need from its first version, because a format cannot grow a field retroactively.

### 15.1 What a `.jbk` is (Realisation IX)

```
  0..15   magic                16 bytes
 16..19   container format     uint32   — versions the container alone
 20..23   header length        uint32
 24..31   payload length       uint64
 32..63   SHA-256 of header    32 bytes
 64..95   SHA-256 of payload   32 bytes
 96..     header               UTF-8 JSON
   ...    payload              the SQLCipher database
```

**Three things version independently** and conflating any two of them would break a vault that only one of them changed: the container format above, the key envelope's own `format` (§4.1), and the database's `user_version`. The header carries all three.

**The payload is the database, not a re-encryption of it.** `VACUUM main INTO` on the open connection, whose output inherits the DEK, keeps `user_version`, and passes `integrity_check`. Re-sealing would mean a second key derivation over a key Argon2id has already stretched, and a second copy of the plaintext in memory to do it with. The three candidates were measured rather than reasoned about before this was chosen (`docs/realisation-ix.md`).

**The header is deliberately cleartext**, and carries the key envelope exactly as `jadeite.keys` holds it — which is what makes §4.4's second row true: a backup opens with the credentials that were in force when it was taken, and nothing else can open it. What the header discloses to someone holding the file and no credential is that a JADEITE vault exists, when this copy was made, which build wrote it, how many times its password has been reset, and when each of four sections was last edited. No amount, no name, no date belonging to a transaction. One number outside the header says more than the header does: the declared payload length correlates with row count, and is the only field in a `.jbk` disclosing a *magnitude* of how much has been recorded. Recorded rather than padded away.

**Every header field the interface displays is bounded.** `appVersion`, `createdAt` and the four stamps are the only fields shown to the owner, and they are shown on the restore confirmation — the one screen whose job is the explicit confirmation this section requires. A header may be 64 KiB; a version string and a timestamp may not, and the timestamps are shape-checked as well as capped. A container carrying an `appVersion` that reads *"verified, no credential required"* is refused as malformed rather than rendered beside the credential box.

**A container from the future is refused.** `migrate` walks forward only and silently ignores a version it has never heard of, so without an explicit gate a laptop running an older build would open a newer vault, skip every migration, and misread it with no error anywhere. That is precisely the rig ↔ laptop case backups exist for.

**Restore is verified before it is applied, and applied so it cannot half-happen.** The database is staged beside the vault, opened, key-proved, integrity-checked and version-checked while it is still a file nothing depends on; only then does the session end and the swap begin. A vault is two files and no filesystem renames two files as one act, so the swap is made *replayable* rather than atomic: a journal is written and flushed before the first rename, and an interrupted install is completed at the next start. The vault being replaced is copied aside first, never deleted — the same ruling as the orphaned database of §4.2.
...
### 17.1 The Hakkında page *(added 31 July 2026, point revision v0.9b)*

The application states what it is, in the rail's foot beside Ayarlar and Yedekleme. It shows the mark, the maker, the version, the release date, where the source lives, and the licence — the last in full, not in summary, opened in place from the page rather than given a destination of its own.

- **The version and the release date are compiled in, not carried over the bridge.** They are fixed at build time and identical for every vault, and `tests/e2e/hardening.spec.ts` pins the preload surface precisely so that widening it is a decision rather than a habit. `package.json` holds `releaseDate` beside `version`, so the two values that must move together at a release are edited in one place.
- **The addresses are text and are never opened.** §3.3 stands unchanged: `window.open` is denied unconditionally, `will-navigate` admits only this application's own files, and no `shell.openExternal` exists anywhere in the tree. The repository and README addresses are rendered selectable so they can be copied, and the page says so in both languages. A link that the window handler would refuse is worse than no link, because it looks like it should work.
- **The licence on screen is the repository's own `LICENSE`**, inlined at build time — one copy, so the notice cannot drift from the file that governs.
- The motto — *Built with Reason and Passion* — is the owner's own phrasing and is the same string in both catalogues, on the precedent of `common.brand`.
- **The page is where the application first called itself *Ekonomi Defteri* / *Economy Journal*** *(v0.9c)*. The strings the outside world reads — the launcher entry, the package listing, and this document's own `Project:` line — followed at v0.9d. The application is allowed to know what it is before the packaging does; it is not allowed to disagree with itself, and it no longer does. The launcher now carries the name pair as `GenericName` in both languages, which is the key a launcher shows and indexes, while `Comment` says what the application *does*: the freedesktop specification asks that a Comment not restate `Name` or `GenericName`, and JADEITE is the Mozilla to Economy Journal's Web Browser.
- **The mark is 66px**, in the rail and on the six ceremony screens *(v0.9c)*. The rail cannot hold it beside the wordmark — 232px of track leaves the brand block 187px, and seven letters at 0.18em tracking reach that on their own near 36px — so the rail head stacks and its wordmark is capped at 28px. The ceremony panels have 394px and keep the side-by-side row. This is geometry rather than taste, and a test asserts it: `.rail-brand` must fit its own box.
...
| Backup payload | **`VACUUM main INTO`, not a byte copy and not `db.backup()`** — measured against Electron 42's binding rather than assumed. `db.backup()` refuses an encrypted source outright; a raw copy is correct only if the WAL was just checkpointed, so its safety rests on a pragma two lines earlier rather than on the statement itself (§15.1) |
| Merge | **Per-section choice** (Q2, closed 31 July 2026). Not built in Realisation IX; the container carries the lineage id and four edit stamps from its first version, because a format cannot grow a field retroactively (§15, §20) |
| Restore credential | **Which of §4.4's two rows applies is decided by lineage, not by asking.** This vault's own backup opens on the in-memory DEK with no prompt, whatever password was current when it was taken; anything else is a transfer and proves a credential against the container's own envelope — password **or** recovery key, and using the recovery key here does not consume it (§4.3 consumes on *reset*, and this is not one) |
| Restore door | **Outside the lock.** On the lock screen and on first-run, because §4.4's second row is a dead disk and every other route to a backup is behind the vault it is replacing |
...
- ~~**Q2:** what **merge** should mean for `.jbk` import (§15).~~ **Closed 31 July 2026 — merge is per-section choice,** ruled by the owner before Realisation IX designed the container, which is what §20 asked for.

  On import the owner picks, section by section, which vault wins. The two rejected readings both cost more than they bought. **Row-level newest-wins** would have needed an `updated_at` column on every user table, a change to every write path in four already-released sections, and a backfilled timestamp on every row that exists today — a fabricated fact, carried forever, in support of a rule that can discard an edit without showing anyone a screen. **A review screen** needs the same per-row machinery and then a page to sit on top of it. Per-section choice needs a vault lineage id and four timestamps.

  **Merge is not built in Realisation IX** and §15 does not put it there; full replacement is what lands. What the ruling bought immediately is the container's shape: a `.jbk` written this year must still be answerable when the chooser is built, and a format cannot grow a field retroactively. So every container carries the lineage id and the four stamps from the first one ever written. That is the whole reason this had to be settled *before* the container rather than during it, and it is why closing a question that changes no behaviour was still worth doing first.

  Schema v5 supplies both. The stamps are kept by SQLite triggers rather than by the write paths, because a rule enforced in forty functions is a rule that will be forgotten in the forty-first. Nothing is backfilled: a vault that arrives at v5 has no stamps and its containers carry null, which is the honest reading of *nothing recorded an edit time before the triggers existed*.
