...
- **The page is where the application first calls itself *Ekonomi Defteri* / *Economy Journal*** *(v0.9c)*. The strings the outside world reads — the launcher tooltip, the package listing, and this document's own `Project:` line — are deliberately still the older description, and follow at Realisation X. The application is allowed to know what it is before the packaging does; it is not allowed to disagree with itself after X.
