...
**The rig's cold start is closed, by the owner's ruling of 1 August 2026 and by an argument this document has already used once.** The box asks for a *ceiling*, not a figure, and the ceiling was cleared on the **slower** of the two machines: 1152 ms against 1500 ms on the Ryzen 5 3450U, where the rig is a 9800X3D. A measurement on the rig can only come in lower, so running it would confirm the box rather than decide it, and a box that cannot fail is not evidence. Recorded rather than quietly ticked, because the reasoning is the whole of the claim: the figure is from the packaged tree rather than from `/opt`, which is the same payload at a different path, and it cleared with 348 ms to spare on hardware six years older than the machine the specification names.

**One item stays open on the Linux side, and it is not code.** The two-minute install clock has no instrument anywhere in the tree — it is a `date +%s` pair run by hand, and it wants a real `pacman -U` that nobody has run since. An owner activity, named here rather than left to be remembered.
...
- [ ] The three items carried in from Realisation X, which this list had named in its preamble and never given a box. — Two of the three are closed. **The `post_upgrade` scriptlet**: written, packaged, and read back out of the built `.pacman` by `tests/package/metadata.spec.ts`. **The rig's ≤ 1.5 s cold start**: closed on the owner's ruling, the ceiling having been cleared at 1152 ms on the slower machine — see above, where the argument is set out rather than assumed. **The two-minute install clock** is the one that remains, and it is the only thing on this list still standing between the ladder and a clean end.
