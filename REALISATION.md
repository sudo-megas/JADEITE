...
- [ ] Cold start inside §3.4 on **both** rigs: ≤ 1.5 s on CachyOS, ≤ 3 s on the reference laptop (Ryzen 5 3450U). *(Named by hardware, as §3.4 names it. This box said "the Arch/Niri laptop" until Realisation XI, which is this document's shorthand and not the specification's — and it went stale the day that machine was reinstalled, while §3.4 did not.)*
  ...
  **The laptop half is measured, and it is the rig that is now outstanding.** Realisation X was built and tested on the **Arch/Niri laptop** — `PRETTY_NAME="Arch Linux"`, `XDG_CURRENT_DESKTOP=niri` — not on the main rig, which is a fact worth stating plainly because every number in this rung comes from that machine. The packaged application reaches the lock screen in **721 ms** there, against 696 ms for the same code launched from source, so packaging costs about twenty-five milliseconds. §3.4 allows the laptop **3 s**; `tests/package/packaged.spec.ts` asserts **1.5 s** and passes, which is the rig's ceiling held on the slower machine.
  ...
  What remained at X's tag was therefore the **CachyOS main rig**, whose ≤ 1.5 s had not been measured against an *installed* 1.0.0 package — and, with it, "on a machine that never built it", since the laptop built this. **Both were closed elsewhere, and neither by the rig**: the provenance half by the CachyOS live session recorded above, and the ceiling by the owner's ruling of 1 August at Realisation XI. The rig has still never been measured, which is that ruling's own point rather than a gap in it.
...
**A reference machine has been reinstalled, and the specification is unaffected — which is not what this paragraph said until the Linux wrap-up read §3.4 instead of quoting it.** The laptop every number in X was measured on, including the 721 ms at X's cold-start box, was wiped and reinstalled with **CachyOS + Niri** on 1 August 2026. §3.4 names its second reference machine by hardware alone — *Ryzen 5 3450U / SATA SSD* — and the words *Arch* and *Niri* appear nowhere in it. That shorthand is this document's own. So the hardware clause is the whole clause, it has not changed, and no amendment to the specification is owed; if anything the reinstall moved the machine **onto** the platform Global rule 3 requires, which the Arch installation never was. What did go stale is this document's own shorthand, at the entries above and at `docs/realisation-x.md`. Those are records of runs made while the machine was Arch and are left as they were written; the one standing *check* among them is amended to name the hardware. **The Arch figures stand as history and are not reproducible** — the same hardware under CachyOS at 1.1.0 answers 736 ms on an idle machine, and with three things changed at once the two are not a controlled comparison — though 15 ms apart is a striking way for them to disagree. So §3.4's second reference machine no longer exists as described, and any re-measurement is on a third configuration that is neither of the two the specification names. This rung either re-measures and re-names it, or says plainly that the Arch figures stand as history and are not reproducible. It is not a defect, but it cannot be left unstated — the whole point of naming reference machines is that a number means something without them being present.
...
**And back on Linux the same day, because a port is not finished until the platform it left still works.** The Windows tree was carried across and the whole suite re-run on the laptop — 4 audits, typecheck clean, **516 unit** (2 skipped, both of them the Windows-only half of a pair whose Linux twin runs beside it — `%APPDATA%` for the config directory and `%LOCALAPPDATA%` for the vault; the second was added by this very session, and this line said 1 until an adversarial read of it counted again), **252 vault** (nothing skipped at all), **111 e2e**, **26 packaged**, and the packaged application reaching its lock screen in **736 ms** against the 1500 ms budget. Every `win32` branch the port added was then read from the Linux side rather than trusted: each is a guard around Windows behaviour with the old Linux path left underneath it, `fsyncDirectory` still flushes and still throws here, and the two changes the Windows session flagged as unguarded both hold — a directory standing where `config.json` belongs refuses the rename with `EISDIR`, which is the same write failing at the same function `chmod` used to fail one statement earlier, and the harness's new skip support is skip-on-Windows only, so a Linux run skips nothing it used to run.
...
**The rig's cold start is closed, by the owner's ruling of 1 August 2026 and by an argument this document has already used once.** The box asks for a *ceiling*, not a figure, and the ceiling was cleared on the **slower** of the two machines: **736 ms** against 1500 ms on the Ryzen 5 3450U, where the rig is a 9800X3D. A measurement on the rig can only come in lower, so running it would confirm the box rather than decide it, and a box that cannot fail is not evidence. Recorded rather than quietly ticked, because the reasoning is the whole of the claim: the figure is from the packaged tree rather than from `/opt`, which is the same payload at a different path, and at 1.1.0 rather than the installed 1.0.0 X's box named; the machine is §3.4's reference laptop on its CachyOS reinstallation of that morning; and it cleared with 764 ms to spare on hardware six years older than the machine the specification names.

*(The figure is load-sensitive and this entry first recorded a worse one. Three runs of the same suite on the same build gave **1812 ms** with a package build finishing alongside it, **1152 ms** with a subagent fleet reading the tree, and **736 ms** with the machine otherwise idle — the last being the one recorded, since a cold-start budget is a statement about the application rather than about what else was running. Worth more than the correction: 736 ms sits **15 ms** from the 721 ms X measured on this same hardware under Arch, from an installed 1.0.0 package. The distribution change the entry above worries about turns out to be worth almost nothing at this budget, which is the strongest thing anyone has said for the Arch figures being history that still holds.)*

**Two items stay open on the Linux side, and neither is code.** The two-minute install clock has no instrument anywhere in the tree — it is a `date +%s` pair run by hand, and it wants a real `pacman -U` that nobody has run since. And the Linux→Windows half of the `.jbk` box below wants a container written here and carried across, which no Windows session can produce for itself. Both are owner activities. *(This paragraph said "one item" when it was written, having forgotten the box four lines under it — which is the failure this document exists to refuse, committed by the session that was auditing the document for exactly that.)*

**The owner ran the shipped release, and it answered three of this rung's open questions at once — one of them badly.** 2–3 August 2026, `jadeite-1.1.0-setup.exe` on Windows and the published packages on Linux.

- **The Windows installer works, and the box this list never had is closed.** The owner installed from `setup.exe` itself rather than from `win-unpacked`, and reports the application runs without fault there. That matters more than a green suite: §16 rule 7 is *installer only, no portable build*, so `win-unpacked` — the only thing any Windows test had ever executed — is not a shipping configuration by the specification's own rule. Every Windows user's first contact is with the one artefact nobody had run. Now somebody has.
- **The live price feature was dead, and had been since before the tag.** Every gold row and silver read as having no source; only the two currency pairs priced. `haremaltin` had begun emitting canonical decimals **unquoted** — `"satis":6183.53` where it once wrote `"satis":"6183.53"` — and `parseKurus` accepted strings only. Eight of ten instruments were discarded at the parser. The full account is below; the part that belongs here is that **the application reported itself healthy throughout**, because two quotes is not zero and `quotes.length === 0` was the only test there was.
- **The release published no Linux binary**, so the owner could not install without building, which also blocked the two-minute install clock they were being asked to measure. Corrected the same day; the reasoning that produced the omission is recorded at the tag box below rather than quietly fixed.

**Acceptance**
- [x] Every machine-checkable prior acceptance list passes on Windows 10/11. — 4 audits; **510 unit** (2 skipped: the XDG case and the POSIX-mode case, neither of which can hold there); **250 vault** (2 skipped, same); **111 e2e**; **15 packaged** (10 skipped: `.pacman`/`.deb` members a `--win` build does not produce); typecheck clean. Every skip is counted and printed rather than deleted. *(This footnote read 509 and omitted the e2e run until the Linux wrap-up checked it against 36ab87f's own message. "All prior acceptance lists" was too broad for a tick, besides: X's visual sweep is owner-observed and was passed at 1440p only.)*
- [ ] **The rendering parity spot-check this rung's scope names was not carried out**, and no box was ever written for it — which is how a scope item comes to evaporate from a rung declared complete. Recorded now rather than left as an absence. The formatting property it would have checked is asserted on both platforms by the unit suite and by `scripts/audit-locale.mjs`; what was not done is the *looking*. The Linux half of the material now exists — `npm run sweep:palettes` was re-run at 1.1.0 on 1 August and wrote twenty frames at both densities — so what remains is the same command on Windows and one person comparing twenty pairs. The one known deliberate difference to expect is the monospace stack.
- [ ] ~~A vault created on Linux, moved as `.jbk`, opens on Windows (and back).~~ **Struck on the owner's ruling, 3 August 2026: "no need — no one will transfer data from Linux to Windows for my part."** The box asked for a journey nobody is going to make, and a rung does not get to keep a box open against the life of the person it was written for. What it would have proved is smaller than it reads, which is why striking it costs little: the container is byte-identical by construction — no path field anywhere in it (`src/shared/backup/types.ts` says so and means it), fixed-width little-endian accessors, UTC timestamps, and a SQLite payload whose cipher parameters are pinned in source rather than inherited from a build. Every property of the format is already machine-checked, on both platforms, by the 252 vault tests. What was never exercised is the *transport* — a file on a stick, and two builds meeting across it. That is an integration fact about a USB drive, not a claim about JADEITE, and it is the half the owner has no use for.
...
- [ ] The three items carried in from Realisation X, which this list had named in its preamble and never given a box. — Two of the three are closed. **The `post_upgrade` scriptlet**: written, packaged, and read back out of the built `.pacman` by `tests/package/metadata.spec.ts`. **The rig's ≤ 1.5 s cold start**: closed on the owner's ruling, the ceiling having been cleared at 736 ms on the slower machine — see above, where the argument is set out rather than assumed. **The two-minute install clock** is the last of the three. It is not the last thing on this list — the Linux→Windows `.jbk` direction above is open too, and the two together are what stands between the ladder and a clean end.
- [x] Tag `v1.1`, and `gh release create`. **The application is complete.** — Cut from Linux on 1 August 2026, once `gh` existed on a machine that could reach the repository. `main` was fast-forwarded to this branch, the tag was cut there, and the draft was retargeted from `realisation-xi-windows` to `main` before it was published. The release carries `jadeite-1.1.0-setup.exe`, `jadeite-1.1.0.pacman` and `jadeite_1.1.0_amd64.deb`. **The tag was moved and the release re-cut on 3 August 2026**, once the live-price defect above was fixed: `v1.1` was deleted and re-created at the fixing commit, and all three installers were rebuilt from it — the Linux pair on this machine, the Windows one on the machine that is allowed to build it. The owner ruled that the release keep the name `v1.1` rather than become a point revision, so the number is unchanged and the bytes are not. This document has called that a hazard before and the ruling stands anyway, on a ground that answers it: **the repository is private, and nobody but the owner has ever been able to download either build.** The trap is that two people hold different bytes under one name, and here there is one person. The notes carry the new checksums; the warning they briefly carried, addressed to downloaders who could not exist, was removed. *(It carried only the `.exe` for a day, on the reasoning that Linux binaries were not the practice — `v1.0` carried no asset at all and §3.A has a Linux reader build from source. That was an inference about the owner's intent from a single prior release, and it was wrong: the owner's answer was that not everyone is going to build from source, and it also left them unable to run the two-minute install check they were being asked for. The packages were uploaded on 3 August and §3.B now offers the download first.)* The notes gained the `pacman -U` paragraph: an owner upgrading from 1.0.0 is the one reader for whom this release is not cosmetic.
