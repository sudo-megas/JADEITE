...
- [ ] Tag `v1.1`, and `gh release create`. **The application is complete.** — The owner's to run, and the tag should be cut where the release is published from. The draft carries `jadeite-1.1.0-setup.exe` and no Linux binary, which is the practice rather than an omission — `v1.0` carried no asset at all, and §3.A of the README has a Linux reader build from source. The Linux artefacts are built and verified at 1.1.0 all the same, because a release nobody can package is not one.
