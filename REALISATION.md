...
- [x] Tag `v1.1`, and `gh release create`. **The application is complete.** — Cut from Linux on 1 August 2026, once `gh` existed on a machine that could reach the repository. `main` was fast-forwarded to this branch, the tag was cut there, and the draft was retargeted from `realisation-xi-windows` to `main` before it was published. The release carries `jadeite-1.1.0-setup.exe` and no Linux binary, which is the practice rather than an omission — `v1.0` carried no asset at all, and §3.A of the README has a Linux reader build from source. The Linux artefacts are built and verified at 1.1.0 all the same, because a release nobody can package is not one. The notes gained the `pacman -U` paragraph: an owner upgrading from 1.0.0 is the one reader for whom this release is not cosmetic.
